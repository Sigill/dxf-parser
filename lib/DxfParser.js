...
DxfParser.prototype._parse = function(dxfString) {
	...
	/**
	 * Helper method to parse multiple contiguous elements.
	 *
	 * The parse logic is handled by a user provided parser. This parser must
	 * advance to the next available group when done.
	 *
	 * @param n The number of elements to parse.
	 * @param elementParser The method used to parse an element.
	 */
	var parseMultipleElements = function(n, elementParser) {
		if(!n || n <= 0)
			throw Error('number of elements to parse must be greater than 0');

		var code = curr.code;
		var elements = [];

		for(var i = 0; (i < n) && (curr.code === code); i++) {
			elements.push(elementParser());
		}

		if (elements.length != n)
			throw Error(elements.length + " elements parsed, " + n + " expected");

		return elements;
	}

	/**
	 * Returns the current group value and advance curr to the next available group.
	 *
	 * This method is to be used with parseMultipleElements(), parsing single
	 * values elements.
	 */
	var copyValueAndAdvance = function() {
		var v = curr.value;
		curr = scanner.next()
		return v;
	}
	 ...
	 *
	 * Specifications: https://www.autodesk.com/techpubs/autocad/acadr14/dxf/
	 *
	 * Does not currently supports normal vectors (groups 210, 220 & 230).
	 */
	var parseSPLINE = function() {
		var entity = {
			...
			control_points         : [],
			fit_points             : [],
			knot_tolerance         : 0.0000001,
			control_point_tolerance: 0.0000001,
			fit_tolerance          : 0.0000000001
		};

		var numberOfKnots = 0, numberOfControlPoints = 0, numberOfFitPoints = 0;
		...
		while(!scanner.isEOF()) {
			...
			switch(curr.code) {
				case 70:
					entity.closed   = ((curr.value & 1) != 0);
					entity.periodic = ((curr.value & 2) != 0);
					entity.rational = ((curr.value & 4) != 0);
					// Linear (16) also implies planar
					entity.planar   = ((curr.value & 24) != 0);
					entity.linear   = ((curr.value & 16) != 0);
					curr = scanner.next();
					break;
				case 71:
					entity.degree = curr.value;
					curr = scanner.next();
					break;
				case 72:
					numberOfKnots = curr.value;
					curr = scanner.next();
					break;
				case 73:
					numberOfControlPoints = curr.value;
					curr = scanner.next();
					break;
				case 74:
					numberOfFitPoints = curr.value;
					curr = scanner.next();
					break;
				case 40:
					entity.knots = parseMultipleElements(numberOfKnots, copyValueAndAdvance);
					break;
				case 10: // X coordinate of control point
					entity.control_points = parseMultipleElements(numberOfControlPoints, parsePoint);
					break;
				case 11: // X coordinate of fit point
					entity.control_points = parseMultipleElements(numberOfFitPoints, parsePoint);
					break;
				case 12: // X value of start tangent
					entity.start_tangent = parsePoint();
					break;
				case 13: // X value of end tangent
					entity.end_tangent = parsePoint();
					break;
				case 41:
					entity.weights = parseMultipleElements(numberOfControlPoints, copyValueAndAdvance);
					break;
				case 42:
					if(curr.value !== 0) entity.knot_tolerance = curr.value;
					curr = scanner.next();
					break;
				case 43:
					if(curr.value !== 0) entity.control_point_tolerance = curr.value;
					curr = scanner.next();
					break;
				case 44:
					if(curr.value !== 0) entity.fit_tolerance = curr.value;
					curr = scanner.next();
					break;
				default:
					checkCommonEntityProperties(entity);
